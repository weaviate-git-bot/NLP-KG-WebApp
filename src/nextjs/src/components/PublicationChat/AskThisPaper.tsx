...
function AskThisPaper(props: AskThisPaperProps) {
  ...
  const answerQuestion = async (
    ...
    const query =
        `Answer the new question based on the following paper text or title: ${props.publication}. ` +
  ...
  const createQuestions = async (newMessage: Message) => {
    try {
      if (props.openaikey) {
        ...
        const questionQuery =
          `Please provide three concise follow-up questions that can be answered by the paper titled ${props.publication}. ` +
