...
const Chat: React.FC = () => {
  ...
  const removeNonUsedReferences = (
    ...
    removedCitations.forEach((citationIndex) => {
      // Since citationIndex is from removedCitations which is a number[], it's always a number.
      for (let j = citationIndex; j <= inlineCitations.length; j++) {
  ...
  const preparePublications = (paper_data: any) => {
    ...
    for (
      ...
      const fullText = paper_data?.publications[i]?.properties.fullText;
      ...
      if (fullText !== undefined) {
        publication_text_to_use =
          publication_text_to_use + " fullText: " + fullText;
      }
    ...
    const fetchResponse = async () => {
      if (
        ...
        const lastUpdatedQuestion =
          "\nPlease explain the following concept and answer the last question in the history: " +
          conceptFromOpenApi +
  ...
  useEffect(() => {
    if (isChatLoading && generationState === "fetched response") {
      //console.log('Decide on chat behaviour')
